"""Collect seeded rule/GBDT Mega-Lucario games against the four baselines."""
...
from agents.mega_lucario.gbdt.features import SCHEMA_ID, check_deck
from agents.mega_lucario.replay import inspect_replay, replay_schema, sha256
...
def collect(
    ...
    teacher: str = "mega_lucario",
    model: Path | None = None,
    workers: int = 1,
    resume: bool = False,
) -> dict[str, Any]:
    """Collect or resume seeded teacher matches against each official rule policy."""
    if teacher not in ("mega_lucario", "mega_lucario_gbdt"):
        raise ValueError("Unsupported teacher")
    if (teacher == "mega_lucario_gbdt") != (model is not None):
    ...
    check_deck(list(map(int, (ROOT / "src/agents/mega_lucario/deck.csv").read_text().split())))
    ...
    def pairing(job: tuple[str, int, int, int]) -> list[dict[str, Any]]:
        ...
        if remaining:
            ...
            deck_a = "mega_lucario" if a == "mega_lucario_gbdt" else a
            deck_b = "mega_lucario" if b == "mega_lucario_gbdt" else b
            command = [
                str(binary.resolve()),
                "--agent-a",
                a,
...
def main() -> None:
    ...
    p.add_argument("--output-dir", type=Path, default=ROOT / "outputs/mega_lucario/rule_replays")
    ...
    p.add_argument(
        "--teacher", choices=["mega_lucario", "mega_lucario_gbdt"], default="mega_lucario"
    )
