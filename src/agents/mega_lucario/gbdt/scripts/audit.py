...
from agents.mega_lucario.gbdt.features import NAMES, SCHEMA_ID, action_queries, check_deck
...
def main() -> None:
    ...
    p.add_argument(
        "--model", "--model-dir", dest="model_dir", type=Path, default=ROOT / "models/mega_lucario"
    )
    p.add_argument("--output", type=Path, default=ROOT / "outputs/mega_lucario/gbdt_parity.json")
    p.add_argument("--feature-dir", type=Path)
    p.add_argument("--probe", type=Path, default=ROOT / "build/gbdt_probe")
    a = p.parse_args()
