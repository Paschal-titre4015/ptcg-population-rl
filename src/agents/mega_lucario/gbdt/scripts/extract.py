...
from agents.mega_lucario.gbdt.features import NAMES, SCHEMA, SCHEMA_ID, action_queries, check_deck
...
def extract(
    ...
    for path in paths:
        ...
        if "mega_lucario" not in header["agents"]:
            continue
        for seat, agent in enumerate(header["agents"]):
            if agent == "mega_lucario":
...
def main() -> None:
    ...
    p.add_argument("--replay-dir", type=Path, default=ROOT / "outputs/mega_lucario/rule_replays")
    p.add_argument("--output-dir", type=Path, default=ROOT / "outputs/mega_lucario/gbdt_features")
