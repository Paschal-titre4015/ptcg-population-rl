...
int main(int argc,char** argv) {
    try {
        ...
        for(std::size_t i=0;i<lucario_schema::IDS.size();++i) deck[lucario_schema::IDS[i]]=lucario_schema::COUNTS[i];
        gbdt::Model model(argv[1],lucario_schema::ID,lucario_schema::NAMES,deck);
        int rows=0,width=0;
