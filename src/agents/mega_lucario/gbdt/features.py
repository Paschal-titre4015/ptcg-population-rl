...
from agents.mega_lucario.gbdt.rich_features import append as append_rich
...
def check_deck(deck: Sequence[int]) -> None:
    ...
    if Counter(deck) != DECK_COUNTS:
        raise ValueError("Mega-Lucario deck does not match the feature/model contract")
...
def card_values(card: dict[str, Any] | None, table: dict[int, dict[str, Any]]) -> list[float]:
    ...
    return [
        ...
        int(master["ex"]),
        int(master["megaEx"]),
        int(master.get("weakness") == 6),
        int(master.get("resistance") == 6),
...
def feature_row(
    ...
    *,
    ...
    row.extend(int(option.get("attackId") == i) for i in range(976, 984))
    zones = [
        me["hand"],
        me["discard"],
        me["active"],
    ...
    def ready(c: dict[str, Any], extra: int) -> bool:
        """Check whether the public card can attack with the additional energy."""
        needed = {673: 1, 674: 3, 675: 2, 676: 1, 677: 1, 678: 1}.get(c["id"], 99)
        return (c["id"] != 676 or any(b and b["id"] == 675 for b in me["bench"])) and c.get(
            "energies", []
        ).count(6) + extra >= needed

    fighting = sum(c["id"] == 6 for c in me["discard"] if c)
    damage = {976: 10, 977: 30, 978: 210, 979: 50, 980: 70, 981: 30, 982: 130, 983: 270}.get(
        option.get("attackId"), 0
    )
    if option.get("attackId") == 980 and not any(c and c["id"] == 675 for c in me["bench"]):
        damage = 0
    if damage and op_active and option["attackId"] != 980:
        master = table[op_active["id"]]
        if master.get("weakness") == 6:
            damage *= 2
        elif master.get("resistance") == 6:
            damage -= 30
    op_hp = (op_active or {}).get("hp", 0)
    ko = damage > 0 and op_hp > 0 and damage >= op_hp
    op_prize = card_values(op_active, table)[-1]
    row.extend(
        [
            int(counts[675] > 0 and counts[676] > 0),
            counts[677] + counts[678],
            counts[673] + counts[674],
            sum(ready(c, 0) for c in me["bench"] if c),
            sum(ready(c, 1) for c in me["bench"] if c),
            fighting,
            sum(len(c.get("energies", [])) for c in board),
            min(3, fighting),
            damage,
            damage - op_hp if damage else 0,
            int(ko),
            int(ko and len(me["prize"]) <= op_prize),
        ]
    )
    if not rich:
        return row
