...
namespace lucario_tokens {
using namespace official;
using Matrix=std::vector<std::vector<double>>;
inline int slot(const CardView& c) {
    auto it=std::find(lucario_schema::IDS.begin(),lucario_schema::IDS.end(),c.id);
    return !c.present || it==lucario_schema::IDS.end() ? -1 : int(it-lucario_schema::IDS.begin());
}
...
inline void set_prefix(Matrix& rows,const Observation& obs,const std::vector<int>& prefix){
    ...
    const auto legal=lucario_features::options_for(obs,prefix);
...
inline Matrix tokenize(const Observation& obs,const std::vector<int>& prefix,const CardTableView& table){
    ...
    auto legal=lucario_features::options_for(obs,prefix);Matrix x(20+n,std::vector<double>(198));
    ...
    auto base=lucario_features::row(obs,-1,{},table,false);std::vector<double> g;
    append(g,base,0,20);append(g,base,90,138);append(g,base,346,354);
    ...
    for(int side=0;side<2;++side){const auto& ps=side==0 ? me : op;
        ...
        for(int j=0;j<6;++j){int r=1+6*side+j;x[r][1]=1;if(j>=count_of(board) || !board[j].present)continue;
            ...
            lucario_features::card_values(v,c,table);generic(r,v);
    ...
    for(std::size_t k=0;k<lucario_schema::IDS.size();++k)remaining[k]=lucario_schema::COUNTS[k];
    ...
    for(int i=0;i<n;++i){int r=19+i;const auto& o=sel.option[i];x[r][0]=1;x[r][1]=3;x[r][2]=contains(legal,i);x[r][3]=contains(prefix,i);
        x[r][4]=slot(lucario_features::candidate(obs,o));auto f=lucario_features::row(obs,i,{},table,false);std::vector<double> v;
        append(v,f,22,60);append(v,f,138,176);append(v,f,346,358);generic(r,v);
        if(o.type==13){x[r][166]=1;x[r][169]=1;x[r][195]=float(f[354]/600);x[r][196]=f[356];x[r][197]=f[357];}
    }
