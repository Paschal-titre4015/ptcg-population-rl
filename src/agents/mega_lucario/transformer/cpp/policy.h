...
class TransformerPolicy final : public Agent {
    ...
    std::shared_ptr<const transformer::Model> model;
    ppo::Sampling sampling;
    std::mt19937 rng;
    struct Query {std::vector<int> prefix,options;std::vector<double> scores;int target;double value;};
    lucario_tokens::Matrix last_tokens;
    std::vector<Query> last_queries;
    std::vector<int> last_action;
    bool recorded=false;
    mutable bool has_encoding=false;
    mutable transformer::Model::Prepared prepared;
public:
    ppo::Statistics last;
    TransformerPolicy(const CardTableView& table,const std::vector<int>& deck,std::shared_ptr<const transformer::Model> model, ppo::Sampling sampling={})
        :table(table),model(std::move(model)),sampling(sampling),rng(sampling.seed) {
        lucario_features::check_deck(deck);
        if(!this->model)throw std::runtime_error("mega_lucario_transformer requires a model");
        ...
        while(count_of(action)<obs.select.maxCount){
            ...
            if(options.size()>1 || sampling.enabled){
                if(last_tokens.empty()){
                    last_tokens=lucario_tokens::tokenize(obs,{},table);
                    model->prepare(last_tokens,prepared);encoded=true;
                }
                lucario_tokens::set_prefix(last_tokens,obs,action);
                auto output=model->evaluate(prepared,last_tokens);value=output.value;
                for(int index:lucario_tokens::indices(obs,options))scores.push_back(output.scores[index]);
                last.value=value;
                const double maximum=*std::max_element(scores.begin(),scores.end());
                while(scores[chosen]<maximum-lucario_schema::TRANSFORMER_TIE)++chosen;
            }
            if(sampling.enabled)chosen=ppo::sample(scores,sampling.temperature,rng,last);
            else ++last.queries;
    ...
    void write_training_record(std::ostream& out,const Observation& obs,const std::vector<int>& action) const override {
        ...
        auto rows=last_tokens.empty() ? lucario_tokens::tokenize(obs,{},table) : last_tokens;
        ...
        for(std::size_t q=0;q<last_queries.size();++q){const auto& query=last_queries[q];
            if(q)buffer+=',';
            lucario_tokens::set_prefix(rows,obs,query.prefix);
            buffer+="{\"prefix\":";lucario_json::numbers(buffer,query.prefix);
            buffer+=",\"options\":";lucario_json::numbers(buffer,query.options);
            buffer+=",\"target\":";lucario_json::number(buffer,query.target);
            buffer+=",\"features\":[";
            for(std::size_t i=0;i<rows.size();++i){if(i)buffer+=',';lucario_json::numbers(buffer,rows[i]);}
            ...
            if(scores.empty()){
                ...
                for(int index:lucario_tokens::indices(obs,query.options))scores.push_back(output.scores[index]);
            }
            buffer+="],\"scores\":";lucario_json::numbers(buffer,scores);
            buffer+=",\"value\":";lucario_json::number(buffer,value);buffer+='}';
        }
        buffer+=']';
        if(sampling.enabled){
            buffer+=",\"ppo\":{\"old_logp\":";lucario_json::number(buffer,last.logp);
            buffer+=",\"old_value\":";lucario_json::number(buffer,last.value);
            buffer+=",\"old_entropy\":";lucario_json::number(buffer,last.entropy);
            buffer+=",\"queries\":";lucario_json::number(buffer,last.queries);buffer+='}';
        }
