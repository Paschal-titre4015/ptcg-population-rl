...
inline void check_deck(const std::vector<int>& deck) {
    ...
    for (std::size_t i=0; i<mega_abomasnow_schema::IDS.size(); ++i) expected[mega_abomasnow_schema::IDS[i]]=mega_abomasnow_schema::COUNTS[i];
    if (counts != expected) throw std::runtime_error("Mega Abomasnow ex feature/model deck mismatch");
}
...
inline void card_values(std::vector<double>& row, const CardView& card, const CardTableView& table) {
    ...
    for (double value : std::array<double,15>{1., double(card.is_pokemon), double(card.hp), double(card.maxHp),
         ...
         double(master.stage2), double(master.ex), double(master.megaEx), double(master.weakness==mega_abomasnow_schema::ATTACK_TYPE),
         double(master.resistance==mega_abomasnow_schema::ATTACK_TYPE), double(card.appearThisTurn), double(prizes)}) row.push_back(value);
}
} // namespace mega_abomasnow_features
...
inline std::vector<double> row(const Observation& obs, int option_index, const std::vector<int>& prefix, const CardTableView& table, bool rich=true) {
    ...
    for(const auto* zone : std::array<const std::vector<CardView>*,10>{&me.hand,&me.discard,&me.active,&me.bench,&sel.deck,&single_card,&single_target,&selected,&effect,&context_card}) {
        ...
        for(int id:mega_abomasnow_schema::IDS) add(counts[id]);
    }
    ...
    const auto& basic=mega_abomasnow_schema::BASIC_ENERGY_IDS;
    for(const auto& c:me.discard)discard+=c.present && contains(basic,c.id);
    int pokemon=0,evolutions=0,attackers=0,hand_energy=0;
    for(int id:mega_abomasnow_schema::POKEMON_IDS)pokemon+=counts[id];
    for(const auto& pair:mega_abomasnow_schema::EVOLUTIONS)evolutions+=std::min(counts[pair[0]],hand[pair[1]]);
    for(int id:mega_abomasnow_schema::ATTACKERS)attackers+=counts[id];
    ...
    if(out.size()!=mega_abomasnow_schema::NAMES.size()) throw std::runtime_error("Feature width mismatch");
    return out;
}
} // namespace mega_abomasnow_features
