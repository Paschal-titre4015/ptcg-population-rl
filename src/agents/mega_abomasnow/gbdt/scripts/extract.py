...
from agents.mega_abomasnow.gbdt.features import NAMES, SCHEMA, SCHEMA_ID, action_queries, check_deck
...
def extract(
    ...
    for path in paths:
        ...
        if "mega_abomasnow" not in header["agents"]:
            continue
        for seat, agent in enumerate(header["agents"]):
            if agent == "mega_abomasnow":
        ...
        *(["calibration"] if calibration_fraction else []),
        ...
        for file_index, entry in enumerate(evidence):
            ...
            for record in decisions(path):
                if record["agent"] != "mega_abomasnow":
                    continue
...
def main() -> None:
    ...
    p.add_argument("--replay-dir", type=Path, default=ROOT / "outputs/mega_abomasnow/rule_replays")
    p.add_argument("--output-dir", type=Path, default=ROOT / "outputs/mega_abomasnow/gbdt_features")
