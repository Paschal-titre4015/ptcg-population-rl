...
from agents.mega_abomasnow.gbdt.features import NAMES, SCHEMA_ID, action_queries, check_deck
from agents.mega_abomasnow.gbdt.train import predict_banks
...
def audit(
    ...
    for path in sorted(replay_dir.rglob("game-*.jsonl")):
        ...
        for record in decisions(path):
            if record["agent"] != "mega_abomasnow_gbdt":
                continue
            report["decisions"] += 1
...
def main() -> None:
    ...
    p.add_argument(
        ...
        dest="model_dir",
        type=Path,
        default=ROOT / "models/mega_abomasnow",
    )
    p.add_argument("--output", type=Path, default=ROOT / "outputs/mega_abomasnow/gbdt_parity.json")
    p.add_argument("--feature-dir", type=Path)
    p.add_argument("--probe", type=Path, default=ROOT / "build/mega_abomasnow_gbdt_probe")
    a = p.parse_args()
