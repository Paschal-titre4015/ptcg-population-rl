...
from agents.mega_abomasnow.gbdt.features import (
    DECK_COUNTS,
    IDS,
    SCHEMA,
    SCHEMA_ID,
...
from agents.mega_abomasnow.transformer.contract import SCORE_TIE_TOLERANCE
...
DOMAIN_OFFSET = SCHEMA["domain_offset"]
WIDTH = 198
NAMES = (
...
CONTRACT = dict(
    version="mega_abomasnow-entity-transformer-v2",
    ...
    features="public-mega_abomasnow-entity-v2; conservative deck/prize belief; attack outcomes unknown and flagged uncertain",
)
...
def tokenize(
    ...
    generic(0, base[:20] + base[90:138] + base[(DOMAIN_OFFSET + 0) : (DOMAIN_OFFSET + 8)])
    ...
    for i, o in enumerate(sel["option"]):
        ...
        generic(r, f[22:60] + f[138:176] + f[(DOMAIN_OFFSET + 0) : (DOMAIN_OFFSET + 12)])
        ...
        if o["type"] == 13:
            result[0] = 1
            result[3] = 1
            result[29] = f[(DOMAIN_OFFSET + 8)] / 600
            result[30] = f[(DOMAIN_OFFSET + 10)]
            result[31] = f[(DOMAIN_OFFSET + 11)]
        x[r, 166:198] = result
    x[-1, 0] = 1
    x[-1, 1] = 4
