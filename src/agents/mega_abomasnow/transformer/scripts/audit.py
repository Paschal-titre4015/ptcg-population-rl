...
from agents.mega_abomasnow.gbdt.features import DECK_COUNTS, SCHEMA_ID, action_queries, check_deck
from agents.mega_abomasnow.replay import decisions, inspect_replay, replay_schema, sha256
from agents.mega_abomasnow.transformer.checkpoint import load_checkpoint
from agents.mega_abomasnow.transformer.features import (
    CONTRACT_ID,
    NAMES,
    encode,
    token_indices,
...
from agents.mega_abomasnow.transformer.model import select_logits
from agents.mega_abomasnow.transformer.train import load_split
...
def audit(
    ...
    for path in sorted(replay_dir.rglob("game-*.jsonl")):
        ...
        for seat, name in enumerate(header["agents"]):
            if name == "mega_abomasnow_transformer":
        ...
        for record in decisions(path):
            if record["agent"] != "mega_abomasnow_transformer":
                continue
            used = True
            report["decisions"] += 1
...
if __name__ == "__main__":
    ...
    p.add_argument("--probe", type=Path, default=ROOT / "build/mega_abomasnow_transformer_probe")
