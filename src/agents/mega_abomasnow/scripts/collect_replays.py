"""Collect seeded rule/GBDT Mega Abomasnow ex games against the four baselines."""
...
from agents.mega_abomasnow.gbdt.features import SCHEMA_ID, check_deck
from agents.mega_abomasnow.replay import inspect_replay, replay_schema, sha256
...
def collect(
    ...
    teacher: str = "mega_abomasnow",
    model: Path | None = None,
    workers: int = 1,
    resume: bool = False,
) -> dict[str, Any]:
    """Collect or resume seeded teacher matches against each official rule policy."""
    if teacher not in ("mega_abomasnow", "mega_abomasnow_gbdt"):
        raise ValueError("Unsupported teacher")
    if (teacher == "mega_abomasnow_gbdt") != (model is not None):
    ...
    check_deck(list(map(int, (ROOT / "src/agents/mega_abomasnow/deck.csv").read_text().split())))
    ...
    def pairing(job: tuple[str, int, int, int]) -> list[dict[str, Any]]:
        ...
        if remaining:
            ...
            deck_a = "mega_abomasnow" if a == "mega_abomasnow_gbdt" else a
            deck_b = "mega_abomasnow" if b == "mega_abomasnow_gbdt" else b
            command = [
                str(binary.resolve()),
                "--agent-a",
                a,
...
def main() -> None:
    ...
    p.add_argument("--output-dir", type=Path, default=ROOT / "outputs/mega_abomasnow/rule_replays")
    ...
    p.add_argument(
        "--teacher", choices=["mega_abomasnow", "mega_abomasnow_gbdt"], default="mega_abomasnow"
    )
