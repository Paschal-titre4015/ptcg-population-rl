"""Convert complete C++ stochastic replays to aligned Mega Abomasnow PPO decisions."""
...
from agents.mega_abomasnow.gbdt.features import SCHEMA_ID, action_queries, check_deck
from agents.mega_abomasnow.replay import decisions, inspect_replay, replay_schema, sha256
from agents.mega_abomasnow.transformer.features import CONTRACT_ID, encode, token_indices, tokenize
    ...
    for path in sorted(replay_dir.rglob("game-*.jsonl")):
        ...
        for seat, sampling in enumerate(header["sampling"]):
            if sampling is None:
                continue
            if (
                header["agents"][seat] != "mega_abomasnow_transformer"
