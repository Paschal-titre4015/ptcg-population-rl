"""Convert complete C++ stochastic replays to aligned Iono PPO decisions."""
...
from agents.iono.gbdt.features import SCHEMA_ID, action_queries, check_deck
from agents.iono.replay import decisions, inspect_replay, replay_schema, sha256
from agents.iono.transformer.features import CONTRACT_ID, encode, token_indices, tokenize
    ...
    for path in sorted(replay_dir.rglob("game-*.jsonl")):
        ...
        for seat, sampling in enumerate(header["sampling"]):
            if sampling is None:
                continue
            if (
                header["agents"][seat] != "iono_transformer"
