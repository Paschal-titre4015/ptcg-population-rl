...
from agents.iono.ppo.extract import extract
from agents.iono.ppo.rollout import audit_behavior
from agents.iono.ppo.scripts.initialize import (
    CONTRACT_ID,
    DECK_COUNTS,
    NAMES,
    ROOT,
...
DEFAULTS = json.loads((ROOT / "src/agents/iono/ppo/config.json").read_text())
...
def collect(
    ...
    *,
    ...
    if (
        allocation.get("iono_gbdt", 0) > 0
        and "iono_gbdt" not in opponent_models
    ...
    def run(job: tuple[str, int, int, int]) -> list[dict[str, Any]]:
        ...
        rival = (
            "iono_transformer" if opponent == "self" or opponent in opponent_models else opponent
        )
        names = ["iono_transformer", rival] if seat == 0 else [rival, "iono_transformer"]
        command = [
            str(binary.resolve()),
            "--games",
            str(count),
        ...
        for side, name in enumerate(names):
            ...
            deck = "iono" if name in ("iono_transformer", "iono_gbdt") else name
            command += [
                f"--agent-{letter}",
                name,
                f"--deck-{letter}",
