"""Collect seeded rule/GBDT Iono games against the four baselines."""
...
from agents.iono.gbdt.features import SCHEMA_ID, check_deck
from agents.iono.replay import inspect_replay, replay_schema, sha256
...
def collect(
    ...
    if teacher not in ("iono", "iono_gbdt"):
        raise ValueError("Unsupported teacher")
    if (teacher == "iono_gbdt") != (model is not None):
    ...
    check_deck(list(map(int, (ROOT / "src/agents/iono/deck.csv").read_text().split())))
    ...
    def pairing(job: tuple[str, int, int, int]) -> list[dict[str, Any]]:
        ...
        if remaining:
            ...
            deck_a = "iono" if a == "iono_gbdt" else a
            deck_b = "iono" if b == "iono_gbdt" else b
            command = [
                str(binary.resolve()),
                "--agent-a",
                a,
...
def main() -> None:
    ...
    p.add_argument("--output-dir", type=Path, default=ROOT / "outputs/iono/rule_replays")
    ...
    p.add_argument("--teacher", choices=["iono", "iono_gbdt"], default="iono")
