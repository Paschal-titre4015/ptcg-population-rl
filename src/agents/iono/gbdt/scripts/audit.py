...
from agents.iono.gbdt.bundle import load_bundle
from agents.iono.gbdt.features import NAMES, SCHEMA_ID, action_queries, check_deck
from agents.iono.gbdt.train import predict_banks
...
def main() -> None:
    ...
    p.add_argument(
        "--model", "--model-dir", dest="model_dir", type=Path, default=ROOT / "models/iono"
    )
    p.add_argument("--output", type=Path, default=ROOT / "outputs/iono/gbdt_parity.json")
    p.add_argument("--feature-dir", type=Path)
    p.add_argument("--probe", type=Path, default=ROOT / "build/iono_gbdt_probe")
    a = p.parse_args()
