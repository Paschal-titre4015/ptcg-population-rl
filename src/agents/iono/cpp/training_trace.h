...
inline void gbdt_queries(std::ostream& out, const official::Observation& obs, const std::vector<int>& action,
    ...
    for(std::size_t q=0;q<targets.size();++q) {
        if(q) out<<',';
        const auto options=iono_features::options_for(obs,prefix);
        ...
        for(std::size_t i=0;i<options.size();++i) {
            if(i)out<<',';const auto features=iono_features::row(obs,options[i],prefix,table);numbers(features);
