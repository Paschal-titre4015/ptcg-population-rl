...
int main(int argc,char** argv){
    try {
        ...
        std::map<int,int> deck;for(std::size_t i=0;i<iono_schema::IDS.size();++i)deck[iono_schema::IDS[i]]=iono_schema::COUNTS[i];
        iono_transformer::Model model(argv[1],iono_schema::TRANSFORMER_ID,iono_schema::ID,198,deck);
        iono_transformer::Model::Prepared prepared;
        int count,width;
