...
from agents.iono.gbdt.features import DECK_COUNTS, NAMES, SCHEMA_ID, action_queries, check_deck
...
from agents.iono.transformer.features import (
    CONTRACT,
    CONTRACT_ID,
    token_indices,
    tokenize,
...
def extract_shard(job: tuple[Path, Path, Path, str, int, list[Path]]) -> dict[str, Any]:
    ...
    for path in selected:
        ...
        digest = sha256(path)
        if replay_schema(header) != SCHEMA_ID or "iono_gbdt" not in header["agents"]:
