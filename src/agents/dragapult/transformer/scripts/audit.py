...
from agents.dragapult.gbdt.features import DECK_COUNTS, SCHEMA_ID, action_queries, check_deck
...
from agents.dragapult.transformer.features import (
    CONTRACT_ID,
    NAMES,
    encode,
    token_indices,
...
from agents.dragapult.transformer.model import select_logits
from agents.dragapult.transformer.train import load_split
...
def audit(
    ...
    for path in sorted(replay_dir.rglob("game-*.jsonl")):
        ...
        for record in decisions(path):
            if record["agent"] != "dragapult_transformer":
                continue
            used = True
            report["decisions"] += 1
...
if __name__ == "__main__":
    ...
    p.add_argument("--probe", type=Path, default=ROOT / "build/dragapult_transformer_probe")
