...
int main(int argc,char** argv){
    try {
        ...
        std::map<int,int> deck;for(std::size_t i=0;i<dragapult_schema::IDS.size();++i)deck[dragapult_schema::IDS[i]]=dragapult_schema::COUNTS[i];
        dragapult_transformer::Model model(argv[1],dragapult_schema::TRANSFORMER_ID,dragapult_schema::ID,198,deck);
        dragapult_transformer::Model::Prepared prepared;
        int count,width;
