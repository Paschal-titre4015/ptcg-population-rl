"""Official dragapult rule policy."""

from __future__ import annotations

# Source: https://www.kaggle.com/code/kiyotah/a-sample-rule-based-agent-dragapult-ex-deck
# Local changes: notebook extraction, module-relative deck loading, formatting, and typing.
...
# Local adaptation: bind the official deck to this agent, independent of cwd.
...
my_deck = [int(card_id) for card_id in file_path.read_text().split()]
if len(my_deck) != 60:
    raise ValueError(f"{file_path}: expected exactly 60 card IDs")
...
def add_card_count(card: Card | Pokemon | None, my_index: int) -> None:
    """Subtract each publicly observed physical card from the remaining deck once."""
    if card == None:
        return
...
def set_card_counts(obs: Observation, my_index: int) -> None:
    """Reconstruct remaining deck counts from observable cards and evolution stacks."""
    card_counts.clear()
    serial_set.clear()
    for id in my_deck:
        card_counts[id] += 1
...
def get_card(
    obs: dict[str, Any], area: AreaType, index: int, player_index: int
) -> Pokemon | Card | None:
...
def main_option_proc(obs: Observation, damage: int) -> None:
    """Build the official attack plan for the current MAIN decision."""
    state = obs.current
    select = obs.select
...
def agent(obs_dict: dict[str, Any]) -> list[int]:
    ...
    def attach_score(attach_id: int, pokemon: Pokemon, active: bool) -> int:
        """Score an energy attachment using the official deck plan."""
    ...
    def hand_score(id: int, ignore_count: bool) -> float:
        """Score a hand card using the current board, resources, and deck plan."""
        score = 0
        if id == Dreepy:
            ...
            if can_main_attack and (
                len(op_state.prize) <= 2 or (bench_attacker and len(op_state.prize) <= 4)
            ):
                score = UNNECESSARY
            else:
                max_score = -10000
    ...
    do_switch = not can_main_attack and (
        bench_attacker or (active_id != Budew and field_counts[Budew] >= 1 and state.turn >= 2)
    )
