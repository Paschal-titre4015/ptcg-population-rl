...
from agents.dragapult.ppo.extract import extract
from agents.dragapult.ppo.rollout import audit_behavior
from agents.dragapult.ppo.scripts.initialize import (
    CONTRACT_ID,
    DECK_COUNTS,
    NAMES,
    ROOT,
...
DEFAULTS = json.loads((ROOT / "src/agents/dragapult/ppo/config.json").read_text())
...
def collect(
    ...
    *,
    ...
    if (
        allocation.get("dragapult_gbdt", 0) > 0
        and "dragapult_gbdt" not in opponent_models
    ...
    def run(job: tuple[str, int, int, int]) -> list[dict[str, Any]]:
        ...
        rival = (
            "dragapult_transformer"
            if opponent == "self" or opponent in opponent_models
            else opponent
        )
        names = ["dragapult_transformer", rival] if seat == 0 else [rival, "dragapult_transformer"]
        command = [
            str(binary.resolve()),
            "--games",
            str(count),
        ...
        for side, name in enumerate(names):
            ...
            deck = "dragapult" if name in ("dragapult_transformer", "dragapult_gbdt") else name
            command += [
                f"--agent-{letter}",
                name,
                f"--deck-{letter}",
            ...
            elif name == "dragapult_transformer":
                command += [
                    f"--model-{letter}",
