...
#include "dragapult_schema.h"

namespace dragapult_features {
using namespace official;
inline void check_deck(const std::vector<int>& deck) {
    std::map<int, int> counts;
    for (int id : deck) ++counts[id];
    std::map<int, int> expected;
    for (std::size_t i=0; i<dragapult_schema::IDS.size(); ++i) expected[dragapult_schema::IDS[i]]=dragapult_schema::COUNTS[i];
    if (counts != expected) throw std::runtime_error("Dragapult ex feature/model deck mismatch");
}
...
inline void card_values(std::vector<double>& row, const CardView& card, const CardTableView& table) {
    ...
    for (double value : std::array<double,15>{1., double(card.is_pokemon), double(card.hp), double(card.maxHp),
         ...
         double(master.stage2), double(master.ex), double(master.megaEx), double(master.weakness==dragapult_schema::ATTACK_TYPE),
         double(master.resistance==dragapult_schema::ATTACK_TYPE), double(card.appearThisTurn), double(prizes)}) row.push_back(value);
}
} // namespace dragapult_features
...
inline std::vector<double> row(const Observation& obs, int option_index, const std::vector<int>& prefix, const CardTableView& table, bool rich=true) {
    ...
    for(const auto* zone : std::array<const std::vector<CardView>*,10>{&me.hand,&me.discard,&me.active,&me.bench,&sel.deck,&single_card,&single_target,&selected,&effect,&context_card}) {
        ...
        for(int id:dragapult_schema::IDS) add(counts[id]);
    }
    ...
    const auto& basic=dragapult_schema::BASIC_ENERGY_IDS;
    for(const auto& c:me.discard)discard+=c.present && contains(basic,c.id);
    int pokemon=0,evolutions=0,attackers=0,hand_energy=0;
    for(int id:dragapult_schema::POKEMON_IDS)pokemon+=counts[id];
    for(const auto& pair:dragapult_schema::EVOLUTIONS)evolutions+=std::min(counts[pair[0]],hand[pair[1]]);
    for(int id:dragapult_schema::ATTACKERS)attackers+=counts[id];
    ...
    if(out.size()!=dragapult_schema::NAMES.size()) throw std::runtime_error("Feature width mismatch");
    return out;
}
} // namespace dragapult_features
