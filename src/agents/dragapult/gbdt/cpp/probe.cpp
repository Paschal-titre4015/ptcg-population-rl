...
int main(int argc,char** argv) {
    try {
        ...
        for(std::size_t i=0;i<dragapult_schema::IDS.size();++i) deck[dragapult_schema::IDS[i]]=dragapult_schema::COUNTS[i];
        dragapult_gbdt::Model model(argv[1],dragapult_schema::ID,dragapult_schema::NAMES,deck);
        int rows=0,width=0;
